...
if(save) { ggsave(filename = "../text/PLoSone_2/pics/postEpsilonLambdaListener.pdf", postEpsilonLambdaSpeaker, width = 6, height = 3) }

# posteriors on typeprior
dataPost = data.frame(R0 = typeprior[,1], R1 = typeprior[,2], R2 = typeprior[,3])
dataPost = melt(dataPost)
dataPost$type = factor(dataPost$variable)
postTypesL = ggplot(dataPost, aes(x = value, lty = type, color = type)) + 
  geom_density(size = 1.15) + scale_colour_grey(start = 0, end = .8) + 
  theme_bw() + xlab('probability')
show(postTypesL)
if(save) { ggsave(filename = "../text/PLoSone_2/pics/postTypesListener.pdf", postTypesL, width = 6, height = 3) }
...
p2d = ggplot(dtp, aes(x = R1, y = R2)) + 
  ...
  xlab("population prior R1") + ylab("population prior R2") +
  geom_point(colour="red",alpha=0.1, size = 1.1) +
  geom_abline(intercept = 1, slope = -1) +
  guides(fill=FALSE) +
  theme_bw()
show(p2d)
if(save) { ggsave(filename = "../text/PLoSone_2/pics/posteriorPopulationDistributionListener.pdf", p2d, width = 5, height = 5) }
...
yskip = 0.005 # height of marker for observed counts
...
pPPC = ggplot(dPPC, aes(x = value, color = condition, group = condition)) + 
  ...
  geom_segment(aes(x = obsR[1,1], y = ypos - yskip, xend = obsR[1,1], yend = ypos + yskip), colour=mycols[1]) + # observations  
  geom_segment(aes(x = hdi.list[[2]][1], y = ypos, xend = hdi.list[[2]][2], yend = ypos), colour=mycols[2]) + # HDI interval  
  geom_segment(aes(x = obsR[2,1], y = ypos - yskip, xend = obsR[2,1], yend = ypos + yskip), colour=mycols[2]) + # observations  
  geom_segment(aes(x = hdi.list[[3]][1], y = ypos, xend = hdi.list[[3]][2], yend = ypos), colour=mycols[3]) + # HDI interval
  geom_segment(aes(x = obsR[3,1], y = ypos - yskip, xend = obsR[3,1], yend = ypos + yskip), colour=mycols[3]) + # observations  
  geom_segment(aes(x = hdi.list[[4]][1], y = ypos, xend = hdi.list[[4]][2], yend = ypos), colour=mycols[4]) + # HDI interval
  geom_segment(aes(x = obsR[4,1], y = ypos - yskip, xend = obsR[4,1], yend = ypos + yskip), colour=mycols[4])  # observations  
show(pPPC)
if(save) { ggsave("../text/PLoSone_2/pics/PPCListener.pdf", pPPC, width = 6, height = 3.5 ) }
...
show(plot.tau)
if(save) { ggsave(filename = "../text/PLoSone_2/pics/posteriorReasoningTypesListener.pdf", plot.tau, width = 6, height = 6) }
